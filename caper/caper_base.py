...
class CaperBase:
        ...
        if gcp_service_account_key_json:
            ...
            if env_name in os.environ:
                ...
                if not os.path.samefile(auth_file, gcp_service_account_key_json):
                    logger.warning(
                        'Env var %s does not match with gcp_service_account_key_json. '
                        'Using application default credentials? ',
                        env_name,
                    )
            logger.debug(
                'Adding GCP service account key JSON %s to env var %s', gcp_service_account_key_json, env_name,
            )
    ...
    def create_timestamped_work_dir(self, prefix: str = '') -> str:
        ...
        logger.info(
            'Creating a timestamped temporary directory. %s', work_dir
        )
