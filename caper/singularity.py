...
def find_bindpath(
    json_file: str,
    common_root_search_level: int = DEFAULT_COMMON_ROOT_SEARCH_LEVEL,
) -> str:
    ...
    def find_dirname(s: str) -> tuple[None, bool]:
        u = AbsPath(s)
        if u.is_valid:
            ...
            all_dirnames.append(Path(os.path.realpath(u.uri)).parent)
        return None, False

    _, _ = recurse_json(json_contents, find_dirname)

    all_dnames_incl_parents: set[str] = set(all_dirnames)
    for d in all_dirnames:
        dir_parts = Path(d).parts
        for i, _ in enumerate(dir_parts[common_root_search_level:]):
            d_child = Path(*dir_parts[: i + common_root_search_level])
            all_dnames_incl_parents.add(str(d_child))

    bindpaths: set[str] = set()
