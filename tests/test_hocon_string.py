...
def get_test_dict(with_include=False):
    ...
    if with_include:
        base_dict['HOCONSTRING_INCLUDE_ad5c3c187d5107c099f66681f1896c70'] = (
            'include required(classpath("application"))'
        )
...
def test_to_dict() -> None:
    ...
    assert hs.to_dict(with_include=False) == get_test_multiple_includes(with_include=False)
    assert hs.to_dict(with_include=True) == get_test_multiple_includes(with_include=True)
...
def test_merge() -> None:
    ...
    d1 = get_test_dict()
    d2 = get_test_dict2()
    d3 = get_test_multiple_includes(True)  # noqa: FBT003
